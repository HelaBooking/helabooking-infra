...
1. Install Terraform on local machine (WSL). Enable auto-complete. (https://developer.hashicorp.com/terraform/tutorials/aws-get-started/install-cli).
2. Setup alias for terraform commands.
   - `alias tf='terraform'`
3. Download the kubeconfig file given to the local machine: (also copy the config to Terraform folder as: `/on-prem/cluster-config/kube-config.yaml`)
...
#### Steps (Already Performed - Can skip):
...
2. Configure the backend in Terraform files to use the S3 bucket.
   ```hcl
   terraform {
     backend "s3" {
