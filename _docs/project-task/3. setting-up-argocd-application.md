...
6. **Setup GitHub Webhooks**:
   - Create a GitHub webhook in the `helabooking-manifests` repository with the following settings:
     - Payload URL: `https://argocd.management.ezbooking.lk/api/webhook`
     - Content type: `application/json`
     - Secret: Use the exsisting webhook secret defined in the Argo CD settings. (if not generate new secret and add it `kubectl edit secret argocd-secret -n management` as `webhook.github.secret: "base64-encoded-secret"`)
